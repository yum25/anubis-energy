...
for r in data:
    ...
    y_temp.append(r["avg_gpu_temp_c"] if r["avg_gpu_temp_c"] is not None else float("nan"))
    y_energy.append(r["steady_state_energy_j"])

    # Goodput: output throughput only if P99 latency is within SLO
    within_slo = (
        r["p99_e2el_ms"] is not None
        and r["p99_e2el_ms"] < SLO_P99_MS
    )
...
if not valid_temp.all():
    print(f"Warning: {(~valid_temp).sum()} configs had no temperature data, "
...
with open(temp_model_path,   "wb") as f: pickle.dump(temp_model,   f)
with open(energy_model_path, "wb") as f: pickle.dump(energy_model, f)
