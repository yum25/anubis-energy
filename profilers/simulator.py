...
class _TapasPhysics:
    ...
    # From TAPAS Fig 7: near-linear, MAE < 1°C on A100
    # V100 runs slightly hotter than A100 at same load → nudge intercept up
    ...
    # TAPAS §2.2: polynomial regression of server power vs GPU load
    # Tuned for V100 300W TDP: idle ~80W, full load ~300W
    POW_A = 0.008  # quadratic term
    POW_B = 1.8  # linear term
    POW_C = 80.0  # idle power
    ...
    # ── Throttle threshold ─────────────────────────────────────────────────────
    THROTTLE_TEMP_C = 83.0  # V100 throttles at ~83°C (slightly below A100's 85°C)

    @classmethod
    def inlet_temp(cls, t_outside: float, dc_load_pct: float) -> float:
        """TAPAS Equation 1."""
        return cls.EQ1_A * t_outside + cls.EQ1_B * dc_load_pct + cls.EQ1_C

    @classmethod
    def gpu_temp_steady(cls, t_inlet: float, gpu_load_pct: float) -> float:
        """TAPAS Equation 2 — steady-state GPU temperature."""
        return cls.EQ2_A * t_inlet + cls.EQ2_B * gpu_load_pct + cls.EQ2_C

    @classmethod
    def gpu_power(cls, gpu_load_pct: float) -> float:
        """TAPAS power polynomial model (Watts)."""
        return cls.POW_A * gpu_load_pct**2 + cls.POW_B * gpu_load_pct + cls.POW_C
...
class SimulatedGpuDataSource(DataSource):
    ...
    # ── ML.Energy baseline energy figures (V100-adjusted from A100 numbers) ───
    # Source: ML.Energy benchmark paper Table 2, Llama-2-7B A100 ~0.003 J/tok
    # V100 adjustment factor: ~1.33 (V100 is ~25% less efficient)
    BASE_ENERGY_PER_TOKEN_DECODE_J = 0.004  # decode phase
    BASE_ENERGY_PER_TOKEN_PREFILL_J = 0.008  # prefill is ~2x more energy-intensive

    # ── GPU load as a function of serving config ───────────────────────────────
    # Rough model: each active sequence contributes ~2.5% GPU load at base freq
    # Frequency scaling: load ∝ (freq / max_freq)^0.7 (sublinear — memory-bound)
    BASE_LOAD_PER_SEQ = 2.5  # % GPU utilisation per concurrent sequence
    MAX_FREQ_MHZ = 1530  # V100 max graphics clock
    ...
    def __init__(
        ...
        idle_inlet = _TapasPhysics.inlet_temp(t_outside_c, dc_load_pct)
        idle_temp = _TapasPhysics.gpu_temp_steady(idle_inlet, 5.0)
    ...
    def next_observation(self, interval_s: float) -> GpuObservation:
        ...
        with self._lock:
            ...
            t_inlet = _TapasPhysics.inlet_temp(effective_outside, self._dc_load)

            # Step 4: TAPAS Eq 2 — steady-state GPU temperature target
            t_gpu_steady = _TapasPhysics.gpu_temp_steady(t_inlet, raw_load_pct)
            ...
            for key in [str(i) for i in self.gpu_indices]:
                ...
                if new_temp >= _TapasPhysics.THROTTLE_TEMP_C:
                    new_temp = _TapasPhysics.THROTTLE_TEMP_C
                    throttled = True
                ...
                power_w = _TapasPhysics.gpu_power(raw_load_pct)
